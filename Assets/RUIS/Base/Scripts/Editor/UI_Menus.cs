...
namespace RUIS.UI.Editor
{
    public class UI_Menus : MonoBehaviour
    {
        ...
        public static void CreateUIGroup()
        {
            ...
            GameObject uiGroup = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/RUIS/Base/Prefabs/UI/UI_Group[Prefab].prefab");
            if (uiGroup)
            {
                GameObject createdGroup = (GameObject)Instantiate(uiGroup);
                createdGroup.name = "UI_Group(Canvas)";
            }
            else
            {
